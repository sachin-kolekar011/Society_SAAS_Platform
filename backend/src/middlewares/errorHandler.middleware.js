...
function errorHandlerMiddleware(err, req, res, next) { // eslint-disable-line no-unused-vars
  ...
  logger.log(isKnown && err.isOperational ? 'warn' : 'error', err.message, {
    ...
    userId: req.user?.id,
    path: req.originalUrl,
    details: err.details, // field-level validation errors -- was missing here, only ever reached the client response, making server logs useless for diagnosing exactly this kind of failure
    stack: err.stack,
  });
