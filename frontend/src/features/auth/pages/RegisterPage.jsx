...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Mirrors the backend's message directly -- e.g. "Selected flat was
      // not found. Please contact your society admin." from Phase 6's
      // registration validation. For a generic "Validation failed" (a
      // Zod schema rejection), the specific field-level reason is in
      // error.details -- surfaced here too, since showing only the generic
      // top-level message left this completely undiagnosable from the UI.
      const apiError = err.response?.data?.error;
      const detailText = apiError?.details?.length
        ? ': ' + apiError.details.map((d) => `${d.field} — ${d.message}`).join(', ')
        : '';
      setError((apiError?.message || 'Registration failed. Please try again.') + detailText);
    } finally {
      setIsLoading(false);
    }
  };
